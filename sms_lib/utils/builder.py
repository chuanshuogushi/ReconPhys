...
from sms_lib.utils.config import CN
...
def build_simulator(cfg: CN, xyz=None, rot=None, init_velocity=None, load_g=None, **kwargs):
    """Build a module from config dict.
    ...
    """
    registry = SIMULATOR
    assert xyz != None, "must input data if use build model with init"
    assert isinstance(cfg, CN) and cfg.TYPE is not None
    ...
    if isinstance(obj_type, str):
        ...
        raise TypeError("type must be a str or valid type, but got {}".format(type(obj_type)))
    if rot is None:
        return obj_cls(cfg, xyz=xyz, init_velocity=init_velocity, load_g=load_g)
    else:
        return obj_cls(cfg, xyz=xyz, rot=rot, init_velocity=init_velocity, load_g=load_g)
