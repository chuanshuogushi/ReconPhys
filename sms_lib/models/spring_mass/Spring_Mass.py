...
class Spring_Mass(nn.Module):
    ...
    def __init__(self, cfg, xyz: torch.Tensor, init_velocity=None, load_g=None) -> None:
        ...
        if self.optim_g:
            if load_g is not None:
                ...
                self.g = nn.Parameter(torch.tensor([[0, 0, -9.8]], dtype=torch.float32), requires_grad=True)
        else:
            if load_g is not None:
        ...
        if init_velocity is None:
            self.init_velocity = nn.Parameter(torch.tensor([[0, 0, 0]], dtype=torch.float32),
        ...
        if self.fix_k:
            if self.single_k:
                self.global_k = nn.Parameter(torch.log10(torch.tensor(GLOBAL_K, dtype=torch.float32)),
                                             requires_grad=fitting)
            else:
                self.global_k = nn.Parameter(torch.log10(torch.tensor(GLOBAL_K, dtype=torch.float32)) *
            ...
            self.global_k = nn.Parameter(torch.log10(torch.tensor(GLOBAL_K, dtype=torch.float32)) *
                                         ...
                                         requires_grad=fitting)
        if self.fix_mass:
            self.global_m = nn.Parameter(torch.log10(torch.tensor(GLOBAL_M, dtype=torch.float32)) *
                                torch.ones(self.n_points, dtype=torch.float32),
                                requires_grad=False)
        else:
            self.global_m = nn.Parameter(torch.log10(torch.tensor(GLOBAL_M, dtype=torch.float32)) *
        ...
        if self.damping:
            if self.fix_damp:
                if self.optim_fixed_damp:
                    self.damp = nn.Parameter(torch.exp(torch.tensor(GLOBAL_DAMP, dtype=torch.float32)),
                                             requires_grad=True)
                else:
                    self.damp = nn.Parameter(torch.log10(torch.tensor(GLOBAL_DAMP, dtype=torch.float32)) *
                                             torch.ones_like(self.origin_len, dtype=torch.float32),
                                             requires_grad=False)
            else:
                self.damp = nn.Parameter(torch.log10(torch.tensor(GLOBAL_DAMP, dtype=torch.float32)) *
        ...
        # 
        self.ext_k_log10 = None
        self.ext_fric_raw = None
        self.ext_soft_vector = None
        self.ext_m_log10 = None
        self.ext_rebound_raw = None
        self.ext_g_vec = None
        self.ext_dt_factor = None
        self.ext_damp_log10 = None
    ...
    def training_velocity_setup(self, training_args):
    ...
    def forward(self, xyz_all: torch.Tensor, xyz: torch.Tensor, v: torch.Tensor, frame_id: int, viz=False, **kwargs):
        ...
        if self.fix_k:
            if self.single_k:
                gk_used = (self.ext_k_log10 if self.ext_k_log10 is not None else self.global_k).reshape(1).repeat(self.n_points)
                K = 10**gk_used.reshape(-1, 1).repeat(1, self.k_neighbors)
            else:
                gk_used = (self.ext_k_log10 if self.ext_k_log10 is not None else self.global_k)
                if gk_used.ndim == 0:
                    gk_used = gk_used.repeat(self.n_points)
                K = 10**gk_used.unsqueeze(1).repeat(1, self.k_neighbors)
        else:
            gk_used = (self.ext_k_log10 if self.ext_k_log10 is not None else self.global_k)
            K = 10**gk_used
        ...
        fric_raw = self.ext_fric_raw if self.ext_fric_raw is not None else self.fric_k
        rebound_k = torch.sigmoid(self.rebound_k)
        fric_k = torch.clamp(torch.sigmoid(fric_raw) * 1.2 - 0.1, min=0, max=1)

        K = K / (self.origin_len + self.eps)

        if self.soft_k:
            soft_vec = self.ext_soft_vector if self.ext_soft_vector is not None else self.soft_vector
            if self.n_fix_spring == 0:
                soft_learn = torch.arange(self.k_neighbors).to(self.device) / self.k_neighbors
                soft_learn = 2 - torch.pow(torch.exp(torch.nn.functional.softplus(soft_vec)), soft_learn)
    ...
    def set_physics_params(self, params=None, **kwargs):
        ...
        # 
        if 'k' in params:
            k_value = params['k'].to(dtype=torch.float32, device=device)
            self.ext_k_log10 = torch.log10(k_value)

        if 'fric_k' in params:
            self.ext_fric_raw = params['fric_k'].to(dtype=torch.float32, device=device)

        if 'soft_vector' in params and self.soft_k:
            self.ext_soft_vector = params['soft_vector'].to(dtype=torch.float32, device=device)
    ...
    # 
    #     if 'k' in params:
    #         # k_value = params['k'].clone().detach().to(dtype=torch.float32, device=device)
    #         k_value = params['k'].to(dtype=torch.float32, device=device)
    #         if self.fix_k:
    #             if self.single_k:
    #                 # self.global_k.data = torch.log10(torch.tensor(k_value, dtype=torch.float32))
    #                 self.global_k.data = torch.log10(k_value)
    #             else:
    #                 # self.global_k.data = torch.log10(torch.tensor(k_value, dtype=torch.float32)) * \
    #                 #                     torch.ones(self.n_points, dtype=torch.float32, device=device)
    #                 self.global_k.data = torch.log10(k_value) * torch.ones(self.n_points, dtype=torch.float32, device=device)

    #         else:
    #             # self.global_k.data = torch.log10(torch.tensor(k_value, dtype=torch.float32)) * \
    #             #                     torch.ones_like(self.origin_len, dtype=torch.float32)
    #             self.global_k.data = torch.log10(k_value) * torch.ones_like(self.origin_len, dtype=torch.float32)
    # 
    #     # self.global_k.data = torch.tensor(3.47)*torch.ones(self.n_points, dtype=torch.float32, device=device)
    # 
    #     # if 'm' in params:
    #     #     m_value = params['m'].clone().detach()
    #     #     self.global_m.data = torch.log10(torch.tensor(m_value, dtype=torch.float32)) * \
    #     #                         torch.ones(self.n_points, dtype=torch.float32, device=device)
        
    # 
    #     # if 'damp' in params and self.damping:
    #     #     damp_value = params['damp'].clone().detach()

    #     #     if self.fix_damp:
    #     #         if self.optim_fixed_damp:
    #     #             self.damp.data = torch.exp(torch.tensor(damp_value, dtype=torch.float32))
    #     #         else:
    #     #             self.damp.data = torch.log10(torch.tensor(damp_value, dtype=torch.float32)) * \
    #     #                             torch.ones_like(self.origin_len, dtype=torch.float32)
    #     #     else:
    #     #         self.damp.data = torch.log10(torch.tensor(damp_value, dtype=torch.float32)) * \
    #     #                         torch.ones_like(self.origin_len, dtype=torch.float32)
    ...
    # 
    #     #     rebound_value = params['rebound_k'].clone().detach()
    #     #     if 0 <= rebound_value <= 1:
    # 
    #     #         rebound_value = torch.logit(torch.tensor(rebound_value, dtype=torch.float32))
    #     #     self.rebound_k.data = torch.tensor([rebound_value], dtype=torch.float32, device=device)
        
    # 
    #     if 'fric_k' in params:
    # 
    #         # fric_value = params['fric_k'].clone().detach()
            
    #         # if 0 <= fric_value <= 1:
    # 
    #         #     fric_value = torch.logit((fric_value + 0.1) / 1.2)
    #         self.fric_k.data = torch.tensor([fric_value], dtype=torch.float32, device=device)
    # 
    #     # self.fric_k.data = torch.tensor(-1.1, dtype=torch.float32, device=device)
        
    # 
    #     # if 'g' in params:
    #     #     g_value = params['g'].clone().detach()
    # 
    #     #     if isinstance(self.g, nn.Parameter):
    #     #         self.g.data = torch.tensor(g_value, dtype=torch.float32, device=device)
    #     #     else:
    #     #         self.g = torch.tensor(g_value, dtype=torch.float32, device=device)
    #     # self.g.data = torch.tensor([0,9.8,0])
    #     # self.g.data = torch.tensor([0,0,9.8])
    # 
    #     # if 'k_bc' in params and self.spring_bc:
    #     #     k_bc_value = params['k_bc']
    #     #     self.k_bc.data = torch.log10(torch.tensor(k_bc_value, dtype=torch.float32, device=device))
        
    # 
    #     # if 'stretch_ratios' in params:
    #     #     self.stretch_ratios = params['stretch_ratios']
        
    # 
    #     # if 'ratio_factor' in params:
    #     #     self.ratio_factor = params['ratio_factor']
    ...
    # 
    #     if 'soft_vector' in params and self.soft_k:
    #         self.soft_vector.data = torch.tensor([params['soft_vector']], dtype=torch.float32, device=device)
    # 
    #     # self.soft_vector.data = torch.tensor(-0.6351,dtype=torch.float32, device=device)
    # 
    #     # if 'init_velocity' in params:
    #     #     init_vel = params['init_velocity'].clone().detach()
            
    #     #     if isinstance(self.init_velocity, nn.Parameter):
    #     #         self.init_velocity.data = init_vel
    #     #     else:
    #     #         self.init_velocity = init_vel
    #     # self.ground = 0.26
    # 
    # 
    #     # logger.info(f"Physics parameters set: {list(params.keys())}")
    #     # logger.info(f"init xyz ground-axis min/max: {self.init_xyz[:, self.ground_axis].min().item()}, {self.init_xyz[:, self.ground_axis].max().item()}")
    #     # logger.info(f"self.ground = {self.ground}, g = {self.g if isinstance(self.g, torch.Tensor) else self.g}")
    #     # logger.info(f"origin_len min/max: {self.origin_len.min().item()}, {self.origin_len.max().item()}")
    #     # logger.info(f"global_k(log10) min/max: {self.global_k.min().item()}, {self.global_k.max().item()}")
    #     # logger.info(f"global_m(log10) min/max: {self.global_m.min().item()}, {self.global_m.max().item()}")
