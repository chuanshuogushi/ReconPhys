...
class Spring_Mass(nn.Module):
    ...
    def set_physics_params(self, params=None, **kwargs):
        ...
        if 'fric_k_final' in params:
            f_final = float(torch.as_tensor(params['fric_k_final'], dtype=torch.float32, device=device).item())
            f_final = max(0.0, min(1.0, f_final))
            y = (f_final + 0.1) / 1.2
            y = min(1 - 1e-6, max(1e-6, y))
            raw = math.log(y / (1 - y))
            self.fric_k = torch.tensor([raw], dtype=torch.float32, device=device)
