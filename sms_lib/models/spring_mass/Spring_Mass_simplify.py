...
class Spring_Mass(nn.Module):
    ...
    def __init__(self, cfg, xyz: torch.Tensor, init_velocity=None, load_g=None) -> None:
        ...
        self.n_ctrl = 0
        self.ctrl_xyz = None          # 
        self.ctrl_bind_index = None   # 
        self.ctrl_rest_len = None     # 
        self.ctrl_k = None            # 
        self.ctrl_prev_xyz = None     # 
        self.ctrl_next_xyz = None     # 
        self.ctrl_k_default = GLOBAL_K    # 
        self.ctrl_k_neighbors = 16        # 
        # ===================================
    ...
    def init_controller(self, ctrl_xyz0: torch.Tensor, k_ctrl=None, k_bind=None):
        """
        translated controller:
        ctrl_xyz0: [N_ctrl, 3] translated（translated，translated xyz translated）
        k_ctrl:    translated（translated，translated self.ctrl_k_default）
        k_bind:    translated control translated（translated，translated self.ctrl_k_neighbors）
        """
        device = self.device
        ...
        if ctrl_xyz0.dim() == 2:
            pass
        elif ctrl_xyz0.dim() == 3 and ctrl_xyz0.shape[0] == 1:
            ctrl_xyz0 = ctrl_xyz0[0]
        else:
            raise ValueError("ctrl_xyz0 shape should be [N_ctrl, 3] or [1, N_ctrl, 3]")

        self.n_ctrl = ctrl_xyz0.shape[0]
        self.ctrl_xyz = ctrl_xyz0.to(device).clone()

        if k_bind is None:
            k_bind = self.ctrl_k_neighbors
        if k_ctrl is None:
            k_ctrl = self.ctrl_k_default

        # 
        dist, knn_idx, _ = self.knn(ctrl_xyz0.to(device), self.init_xyz, k_bind, rm_self=False)
        # dist: [N_ctrl, k_bind]
        self.ctrl_bind_index = knn_idx          # [N_ctrl, k_bind]
        self.ctrl_rest_len = dist               # 

        # 
        if isinstance(k_ctrl, (float, int)):
            self.ctrl_k = torch.full_like(self.ctrl_rest_len, float(k_ctrl), device=device)
        else:
            k_ctrl = torch.as_tensor(k_ctrl, dtype=torch.float32, device=device)
            if k_ctrl.ndim == 0:
                self.ctrl_k = torch.full_like(self.ctrl_rest_len, float(k_ctrl), device=device)
            elif k_ctrl.numel() == self.n_ctrl:
                self.ctrl_k = k_ctrl.view(self.n_ctrl, 1).repeat(1, k_bind)
            else:
                raise ValueError("k_ctrl shape mismatch")
    ...
    def set_all_particle(self, xyz_all: torch.Tensor):
        ...
        # 
        intrp_len, self.intrp_index, _ = self.knn(self.init_xyz_all, self.init_xyz, self.k_binding)  # [N_all, k]
        # 
        sigma = intrp_len[:, -1:].clamp(min=1e-6)  # [N_all, 1]
        w = torch.exp(-(intrp_len ** 2) / (2 * sigma ** 2))  # [N_all, k]
        self.intrp_coef = w / (w.sum(dim=-1, keepdim=True) + self.eps)  # 
    ...
    def interpolate(self, xyz_all: torch.Tensor, xyz_before: torch.Tensor, delta_xyz: torch.Tensor):
        """
        translated：translated2048translated delta_xyz，translated，
        translated。translated(translated)translated，translated，translated。
        """
        delta_knn = delta_xyz[self.intrp_index]  # [N_all, k, 3]
        delta_xyz_all = torch.sum(delta_knn * self.intrp_coef.unsqueeze(-1), dim=1)  # [N_all, 3]
    ...
    def compute_ctrl_force(self, xyz, v):
        """
        translated controller translated（controller translated）
        xyz: [N, 3] translated
        v:   [N, 3] translated（translated，translated dashpot translated）
        return: [N, 3] translated
        """
        if (not self.has_controller) or self.n_ctrl == 0:
            return torch.zeros_like(xyz, device=self.device)

        device = self.device
        N = xyz.shape[0]
        F_obj = torch.zeros_like(xyz, device=device)

        # ctrl_xyz: [N_ctrl, 3]
        ctrl_pos = self.ctrl_xyz                   # [Nc, 3]
        idx = self.ctrl_bind_index                 # [Nc, Kc]
        rest = self.ctrl_rest_len                  # [Nc, Kc]
        k = self.ctrl_k                            # [Nc, Kc]

        obj_pos = xyz[idx]                         # [Nc, Kc, 3]
        ctrl_pos_exp = ctrl_pos.unsqueeze(1).expand_as(obj_pos)  # [Nc, Kc, 3]

        dis = obj_pos - ctrl_pos_exp               # 
        cur_len = torch.norm(dis, dim=-1)          # [Nc, Kc]
        dir = dis / (cur_len.unsqueeze(-1) + self.eps)  # [Nc, Kc, 3]

        stretch = cur_len / (rest + self.eps) - 1.0    # [Nc, Kc]
        F_spring = k.unsqueeze(-1) * stretch.unsqueeze(-1) * dir     # [Nc, Kc, 3]

        # 
        F_obj.index_add_(0, idx.reshape(-1), F_spring.reshape(-1, 3))
    ...
    def step(self, xyz, v, K, m, rebound_k, fric_k, damp, dt):
        ...
        if self.has_controller:
            force_ctrl = self.compute_ctrl_force(xyz, v)
    ...
    def forward(self, xyz_all: torch.Tensor, xyz: torch.Tensor, v: torch.Tensor, frame_id: int, viz=False, **kwargs):
        ...
        for si in range(self.n_step):
            # 
            if self.has_controller and (self.ctrl_prev_xyz is not None):
                self._interp_controller(si, self.n_step)
            # ===================================
            xyz, v = self.step(xyz=xyz, v=v, K=K, m=m, rebound_k=rebound_k, fric_k=fric_k, damp=damp, dt=dt)
            torch.cuda.empty_cache()
        # 
    ...
    def set_physics_params(self, params=None, **kwargs):
        ...
        # 
        # 
        if 'fric_k_final' in params:
            f_final = torch.as_tensor(params['fric_k_final'], dtype=torch.float32, device=device)
            f_final = torch.clamp(f_final, 0.0, 1.0)
            y = (f_final + 0.1) / 1.2
            y = torch.clamp(y, 1e-6, 1 - 1e-6)
            raw = torch.log(y / (1 - y))
            self.fric_k = raw.reshape(1)
        ...
        if 'damp' in params and self.damping:
            ...
            if self.fix_damp and self.optim_fixed_damp:
                ...
                elif damp_value.shape == (1, self.origin_len.shape[0], self.origin_len.shape[1]):
                    self.damp = torch.log10(damp_value.squeeze(0))
                else:
    ...
    def set_controller_target(self, prev_ctrl_xyz: torch.Tensor, next_ctrl_xyz: torch.Tensor):
        """
        translated controller translated & translated，translated forward translated
        translated: [N_ctrl, 3]
        """
        assert self.has_controller, "call init_controller first"
        assert prev_ctrl_xyz.shape[0] == self.n_ctrl
        assert next_ctrl_xyz.shape[0] == self.n_ctrl
        self.ctrl_prev_xyz = prev_ctrl_xyz.to(self.device).clone()
        self.ctrl_next_xyz = next_ctrl_xyz.to(self.device).clone()

    def _interp_controller(self, substep: int, n_substep: int):
        """
        translated substep translated，translated controller translated
        """
        if self.ctrl_prev_xyz is None or self.ctrl_next_xyz is None:
            # 
            return
        t = float(substep + 1) / float(n_substep)
        self.ctrl_xyz = self.ctrl_prev_xyz + (self.ctrl_next_xyz - self.ctrl_prev_xyz) * t

    def set_controller_pos(self, ctrl_xyz: torch.Tensor):
        """
        translated，translated ctrl translated（translated/translated）
        """
        assert self.has_controller, "call init_controller first"
        assert ctrl_xyz.shape[0] == self.n_ctrl
        self.ctrl_xyz = ctrl_xyz.to(self.device).clone()