...
class Gaussian:
    ...
    #     rot_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("rot")]
    #     rot_names = sorted(rot_names, key = lambda x: int(x.split('_')[-1]))
    #     rots_np = np.zeros((xyz.shape[0], len(rot_names)))
    #     for idx, attr_name in enumerate(rot_names):
    #         rots_np[:, idx] = np.asarray(plydata.elements[0][attr_name])
    ...
    #         xyz = np.matmul(xyz, T)
    # 
    #         rot_mats = utils3d.numpy.quaternion_to_matrix(rots_np)  # (N, 3, 3)
    # 
    #         rots_np = utils3d.numpy.matrix_to_quaternion(rot_mats)  # (N, 4)
    ...
    #     opacities = torch.sigmoid(torch.tensor(opacities, dtype=torch.float, device=self.device))
    #     scales = torch.exp(torch.tensor(scales, dtype=torch.float, device=self.device))
    #     rots = torch.tensor(rots_np, dtype=torch.float, device=self.device)
    ...
    def _sanitize(self):
        ...
        for k in ['_xyz', '_features_dc', '_features_rest', '_scaling', '_rotation', '_opacity']:
            ...
            if t is not None:
                setattr(self, k, t.to(self.device, dtype=torch.float32).contiguous())
        # 
        if self.aabb.device.type != self.device:
            self.aabb = self.aabb.to(self.device)
        # 
        if self._rotation is not None:
            self._rotation = torch.nn.functional.normalize(self._rotation + self.rots_bias[None, :]) - self.rots_bias[None, :]

    def validate(self):
        def _finite(x): return x is not None and torch.isfinite(x).all().item()
        assert _finite(self._xyz), "xyz has NaN/Inf"
        assert _finite(self._scaling), "scaling has NaN/Inf"
        assert _finite(self._rotation), "rotation has NaN/Inf"
        assert _finite(self._opacity), "opacity has NaN/Inf"
        assert self._xyz.is_cuda, "xyz must be on CUDA"
        assert self._scaling.is_cuda and self._rotation.is_cuda and self._opacity.is_cuda, "tensors must be on CUDA"
    ...
    def load_ply(self, path, transform=[[1, 0, 0], [0, 0, -1], [0, 1, 0]]):
        ...
        rot_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("rot")]
        rot_names = sorted(rot_names, key=lambda x: int(x.split('_')[-1]))
        rots_np = np.zeros((xyz.shape[0], len(rot_names)), dtype=np.float32)
        for idx, attr_name in enumerate(rot_names):
            rots_np[:, idx] = np.asarray(plydata.elements[0][attr_name])
        ...
        if transform is not None:
            T = np.asarray(transform, dtype=np.float32)
            xyz = np.matmul(xyz, T)  # save: xyz_out = xyz @ T.T
            rot_mats = utils3d.numpy.quaternion_to_matrix(rots_np)  # save: R_out = T @ R
            rot_mats = np.matmul(T.T, rot_mats)                     #       R = T.T @ R_out
            rots_np = utils3d.numpy.matrix_to_quaternion(rot_mats)

        # 
        xyz = torch.tensor(xyz, dtype=torch.float32, device=self.device).contiguous()
        features_dc = torch.tensor(features_dc, dtype=torch.float32, device=self.device).transpose(1, 2).contiguous()  # (N,1,3)
        if self.sh_degree > 0:
            features_extra = torch.tensor(features_extra, dtype=torch.float32, device=self.device).transpose(1, 2).contiguous()
        opacities = torch.sigmoid(torch.tensor(opacities, dtype=torch.float32, device=self.device))
        scales = torch.exp(torch.tensor(scales, dtype=torch.float32, device=self.device))
        rots = torch.tensor(rots_np, dtype=torch.float32, device=self.device)

        # 
        self._xyz = (xyz - self.aabb[None, :3]) / self.aabb[None, 3:]
        self._features_dc = features_dc
        self._features_rest = features_extra if self.sh_degree > 0 else None
        self._opacity = self.inverse_opacity_activation(opacities) - self.opacity_bias
        self._scaling = self.inverse_scaling_activation(torch.sqrt(torch.clamp_min(scales**2 - self.mininum_kernel_size ** 2, 1e-12))) - self.scale_bias
        self._rotation = rots - self.rots_bias[None, :]
