...
def render(viewpoint_camera, pc : Gaussian, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    # assert torch.isfinite(means3D).all(), f"means3D has NaN/Inf: {torch.isnan(means3D).sum()} NaN, {torch.isinf(means3D).sum()} Inf"
    # assert torch.isfinite(opacity).all(), "opacity has invalid values"
    # assert torch.isfinite(scales).all(), "scales has invalid values"
    # assert torch.isfinite(rotations).all(), "rotations has invalid values"
    ...
    # 
    # assert radii is not None, "radii is None"
    # assert radii.numel() > 0, "radii is empty"
    # assert radii.is_cuda, "radii not on GPU"
    # assert radii.shape[0] == means3D.shape[0], f"radii length {radii.shape[0]} != gaussians {means3D.shape[0]}"

    # 
    # if not torch.isfinite(radii).all():
    #     print("⚠️ radii contains NaN/Inf! Debug info:")
    #     print("radii min/max:", radii.min().item(), radii.max().item())
    #     print("NaN count:", torch.isnan(radii).sum().item())
    #     print("Inf count:", torch.isinf(radii).sum().item())
    # 
